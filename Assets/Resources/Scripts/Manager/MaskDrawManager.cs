...
public class MaskDrawManager : BaseMonoBehaviour
{
    ...
    private void Update() {
        ...
        if (mouseDrag && penType == PenType.Pen) {
            PenDraw();
        }else {
            //鼠标没有按住
    ...
    private void PenDraw() {
        ...
        if (hit.collider != null) {
            if (isDraging==false) {//刚按下鼠标左键，还没拖动
                previousColorArray = maskTexture2D.GetPixels32();
            }
            //正在滑动
            isDraging = true;
            //鼠标位置转化为对应sprite像素位置        
            ...
            for (float lerp = 0; lerp <= 1; lerp += steps)
            {
                ...
                //画
                MarkPixelsToColour(curPosition, penWidth);
            }
            previousDragPosition = pixelPos;
            maskTexture2D.SetPixels32(currentColorArray);
            maskTexture2D.Apply();
        ...
        for (int x = centerX - penWidth; x <= centerX + penWidth; x++)
        {
            ...
            for (int y = centerY - penWidth; y <= centerY + penWidth; y++)
            {
                ...
                if (x >= spriteWidth || x < 0 ||
                    y>= spriteHeight|| y <0 )
                    continue;
                int arrayPos = y * (int)maskSprite.rect.width + x;
    ...
    private Vector2 WorldToPixelCoordinates(Vector3 mouseWorldPosition) {
        ...
        float pixelWidth = spriteWidth;//512
        float pixelHeight = spriteHeight;//512
   ...
   private void Brush() {
    ...
    private void FloodFillScanLineWithStack(int x, int y, Color32 newColor, Color32 oldColor) {
