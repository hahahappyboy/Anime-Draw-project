...
public class InstanceZoneManager : MonoBehaviour{
    # region 组件
    //档案的父物体
    ...
    private void Awake() {
        ...
        FetchComponent();
        //得到档案数据
        GetInstanceZoneInfoByLocalResources();
    ...
    private void InitZoneLayout() {
        ...
        for (int i = 0; i < instanceZoneInfoList.Count; i++) {
            ...
            Sprite zoneImageSprite = Resources.Load<Sprite>(instanceZoneInfoList[i].zoneImageURL);
            zoneController.SetZoneInfo(
                zoneImageSprite,
                instanceZoneInfoList[i].zoneTextCN,
                instanceZoneInfoList[i].zoneTextJP,
                instanceZoneInfoList[i].isLock);
            //加入循环链表
    ...
    /// 获取本地档案信息
    /// </summary>
    private void GetInstanceZoneInfoByLocalResources() {
        //获取本地档案Json
